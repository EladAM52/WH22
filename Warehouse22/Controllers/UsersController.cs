...
namespace WareHouse22.Controllers
{
    public class UsersController : Controller
    {
        ...
        /*-----------------3 functions that check if item can be loan in those dates------------*/
        public Boolean CheckLoanAvailableDates(string Code, string StartDate, string ReturnDate)
        {
            ...
            {
                if (l.ECode == Code && ((Start >= l.StartDate && Start <= l.FinalDate) || (End >= l.StartDate && End <= l.FinalDate) || (Start <= l.StartDate && End >= l.FinalDate)))
                    return false;
            }
            return true;
        }
        ...
        public ActionResult ChangeLoanItem(string Code, string Email, string StartDate, string ReturnDate, string newStartDate, string newReturnDate)
        {
            ...
            if (CheckLoanAvailableDates(Code, newStartDate, newReturnDate) &&
                ...
                CheckDatesFormat(newStartDate, newReturnDate))
            {
        ...
        /*-----------------Edit existing item fields------------*/
        [HttpPost]
        public ActionResult EditItem(string Code, string Name, string State, string Category, string Instruction, string MaxLoanTime)
        {
            ...
            if (CheckItemExists(Code) == true)
            {
                ...
                foreach (tblEquipment u in Equipmentdb.Equipment)
                {
                    if (u.Code == Code)
                    {
                        ...
                        else ed.MaxLoanTime = Int32.Parse(MaxLoanTime);
        ...
        public ActionResult ShowReports()
        {
            ...
            foreach (tblLoans u in Loansdb.Loans)
            {
                if ((u.StartDate >= DateTime.Today || u.FinalDate == DateTime.Today) && u.Returned.Equals("F")) { NumCurrentLoans++; }
                foreach (tblLoans l in Loansdb.Loans)
                {
        ...
        public ActionResult ReturnItem(string userEmail, string code, string startDate, string finalDate)
        {
            try
            {
                ...
                foreach (tblLoans u in Loansdb.Loans)
                {
                    if (u.UserEmail.Equals(userEmail) && u.StartDate == Start && u.FinalDate == End && u.ECode.Equals(code))
                    {
                        //AddReturnLoan(userEmail, code, startDate,finalDate);
                        u.Returned = "T";
                    }
                }
                Loansdb.SaveChanges();
        ...
        [HttpPost]
        public ActionResult CheckUnSeenNotification(string mail)
        {
            mail = mail.ToLower();
            foreach (tblNotification u in Notificationdb.Notification)
            {
                if (u.Email.ToLower().Equals(mail) && u.Seen.Equals("F"))
                {
                    return Json(new { status = "true" });
                }
            }
        ...
        [HttpPost]
        public ActionResult ChangePass(string mail, string NewPassword)
        {
            int flag = 0;
            mail = mail.ToLower();
            foreach (tblUsers u in db.Users)
            {
                if (u.Email.ToLower().Equals(mail))
                {
        ...
        /*-----------------Show items that avillable in specific dates------------*/

        [HttpPost]
        public ActionResult ShowEquipmentDate(string Category, string SearchStartDate, string SearchFinalDate)
        {
            ...
            foreach (tblEquipment u in Equipmentdb.Equipment)
            {
                if (u.Category.ToLower().Equals(Category))
                    if (CheckLoanAvailableDates(u.Code, SearchStartDate, SearchFinalDate) && CheckDatesFormat(SearchStartDate, SearchFinalDate))
                        equipmentlist.Add(u);
            }
            f.ShowEquipments = equipmentlist.ToList();
            return Json(new { equipmentlist });
        ...
        [HttpPost]
        public ActionResult ChangeAdminHome()
        {
            if (AdminFlag == 'F')
            {
                AdminFlag = 'T';
